...
type ServiceTabId = "IMPS" | "UPI" | "Recharge" | "DMT" | "BillPayments" | "CreditCardPayment" | "Transactions";
...
const ServicesPage = () => {
  const [selectedService, setSelectedService] = useState<ServiceTabId | null>(null); // null for grid view
  // activeServiceTab is now derived from selectedService or managed internally for sub-content
  ...
  const [newBankBeneficiaryData, setNewBankBeneficiaryData] = useState({
    accountNumber: "", confirmAccountNumber: "", accountHolderName: "", ifscCode: "", transactionType: "IMPS" as "NEFT" | "IMPS",
  });
  ...
  const fetchBankBeneficiaries = useCallback(async (showLoading = true) => { /* ... (same as before, using dummy data) ... */
    if (showLoading) setLoading(true);
    const dummyData: BankBeneficiary[] = [ { id: 'bank1', accountNumber: '123456789012', accountHolderName: 'Alice Smith (Bank)', transactionType: 'IMPS', isVerified: true, createdAt: new Date().toISOString(), userId: 'user1', bankName: 'Global Bank', ifscCode: 'GBIN0001234' }, { id: 'bank2', accountNumber: '098765432109', accountHolderName: 'Bob Johnson (Bank)', transactionType: 'NEFT', isVerified: false, createdAt: new Date().toISOString(), userId: 'user1', bankName: 'National Bank', ifscCode: 'NBIN0005678' }, ];
    setBankBeneficiaries(dummyData);
    if (showLoading) setLoading(false);
  }, []);
  const fetchUpiBeneficiaries = useCallback(async (showLoading = true) => { /* ... (same as before, using dummy data) ... */
    if (showLoading) setLoading(true);
    const dummyData: UpiBeneficiary[] = [ { id: 'upi1', upiId: 'alice@okbank', accountHolderName: 'Alice Smith (UPI)', isVerified: true, createdAt: new Date().toISOString(), userId: 'user1' }, { id: 'upi2', upiId: 'bob.john@okaxis', accountHolderName: 'Bob J.', isVerified: true, createdAt: new Date().toISOString(), userId: 'user1' }, ];
    setUpiBeneficiaries(dummyData);
    if (showLoading) setLoading(false);
  }, []);
  const fetchTransactions = useCallback(async () => { /* ... (same as before, using dummy data) ... */
    setLoading(true);
    const dummyTransactions: Transaction[] = [ {id: 't1', amount: 100, chargesAmount: 2, transactionType: 'IMPS', transactionStatus: 'COMPLETED', transactionTime: new Date().toISOString(), beneficiaryName: 'Alice Smith (Bank)', beneficiaryIdentifier: '...9012'}, {id: 't2', amount: 50, chargesAmount: 1, transactionType: 'UPI', transactionStatus: 'PENDING', transactionTime: new Date().toISOString(), beneficiaryName: 'Bob J. (UPI)', beneficiaryIdentifier: 'bob.john@okaxis'}, ];
  ...
  useEffect(() => {
    ...
    setSearchQuery(""); // Reset search when service selection changes
  ...
  const handleServiceSelect = (serviceId: ServiceTabId) => {
    ...
    // Reset sub-tabs when a new main service is selected
  ...
  // --- Form Handlers (Bank & UPI - same as before) ---
  ...
  // --- Sub Tab Button Component (same as before) ---
  ...
  return (
    <MainLayout location="/services">
      ...
      <div className="space-y-6">
        ...
        {/* Content Area: Grid or Selected Service */}
        <AnimatePresence mode="wait">
          {!selectedService ? (
            ...
            >
              {services.map((service) => (
                <motion.button
                  ...
                  className="bg-card border border-border/40 rounded-xl p-4 md:p-6 text-left hover:shadow-lg hover:border-primary/50 transition-all duration-200 group focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background"
                  whileHover={{ y: -5 }}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.2, delay: services.indexOf(service) * 0.05 }}
                >
                  ...
                  <p className="text-xs md:text-sm text-muted-foreground line-clamp-2"> {/* line-clamp for consistent height */}
                    {service.description}
                  </p>
                </motion.button>
              ))}
            ...
            <motion.div
              key={selectedService} // Important for AnimatePresence to detect change
              initial={{ opacity: 0, x: selectedService ? 30 : -30 }}
              animate={{ opacity: 1, x: 0 }}
              exit={{ opacity: 0, x: selectedService ? -30 : 30 }}
